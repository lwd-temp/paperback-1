...
func Encrypt(plaintext, key []byte, headers map[string]string) (cipher Packet, err error) {
	...
	cipher.Extra.Version = version.Version
...
func Decrypt(cipher Packet, key []byte) (plaintext []byte, headers map[string]string, err error) {
	...
	if cipher.Extra.Version != version.Version {
		// TODO: In future we should add backwards compatibility.
		return nil, nil, errors.Errorf("unknown version %s", cipher.Extra.Version)
	}
